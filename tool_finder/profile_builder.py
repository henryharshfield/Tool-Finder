...
def build_fastener_profile(source_profile: dict) -> FastenerProfile:
    fastener_family = normalize_family(source_profile["fastener_family"])
    if not is_supported_fastener_family(fastener_family):
        raise ValueError(f"Unsupported v1 family: {fastener_family}")
